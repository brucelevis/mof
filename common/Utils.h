...
public:
    ...
    static time_t mktimeFromWday(int wday, int hour, int min, int sec = 0);
    ...
    /*
     ...
     ---linshusen
     */
    static bool isSingleUtf8StringIsEmoji(const std::string& subString_utf8);  //当个utf8字符串是不是一个Emoji表情
    ...
    static void separationEmojiFromUtf8String(const std::string& str, vector<string> &StrDatas,vector<int> &isEmojiStr);   //切出字符串 把非表情和表情字符串  放到一个容器里
...
public:
    ...
    //某个消息是不是被锁了
    static bool TcpMsgIsLocked(int msgType);
//~~~~~~~~~~~~~~
//    static int checkCopyOpen(int _copyID,int _lastCopyID);
   

    //TalkingData
    static std::string JobToString(int _job);
//    static std::string SceneToString(int _sceneID);
    ...
    ///根据参数排序并输出排列好的参数
    static std::string getPostPram(vector<std::string>* pram);
    static char contrastASCII(string strA,string strB);
    ...
    //判断两个浮点数是否真的相等---linshusen
    static bool isFloatEqual(float f1, float f2)
    {
        float disparity = f1 - f2;
        if(disparity < 0.000001 && disparity > -0.000001)
            return true;
        else
            return false;
    }
...
int xyJsonAsInt(const Json::Value &val, int defaultValue = 0);
...
void xySetDifferenceMap(InputIterator1 first1, InputIterator1 last1,
						InputIterator2 first2, InputIterator2 last2,
						OutputIterator &result)
{
	while (first1 != last1 && first2 != last2)
	{
		if (first1->first < first2->first) { result.push_back(&first1->second); ++first1;}
		else if (first2->first < first1->first) ++first2;
		else { ++first1; ++first2; }
	}
	while (first1 != last1) {
		result.push_back(&first1->second);
		++first1;
	}
}
...
/***************************************************************
 ...
 返回：
        ...
        num = second - first
 ****************************************************************/
std::pair<int,int> checkPageRange(int size,int start ,int num ,int maxNum = 100);
...
string find_and_replace_all(string& in ,const string& findstr ,const string& replacestr);
//串连接 string int
template <class T>
string stringjoint(vector<T> &vec, const string &sp)
{
    if (vec.size() == 0)
    {
        return "";
