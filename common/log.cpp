...
int log_level_file = LOG_LEVEL_NULL;
int console_level_file = LOG_LEVEL_INFO;
...
void strtime(std::ostream& out)
{
    ...
    out<<ltm.tm_hour << ":"<< ltm.tm_min << ":"<< ltm.tm_sec<<":"<<tv.tv_usec;
}
...
void log_level_up()
{
    if (log_level_file < LOG_LEVEL_TRACE) {
        log_level_file++;
    }
...
bool log_init(int level, const char* name)
{
    ...
    if (!fname.empty()) {
        file_log.open(fname.c_str(), std::ios::app);
    }
    return true;
}
