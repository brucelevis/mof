...
function treasurefight_getPlayerAward(points,lvl)
	...
	if constrib >30*lvl^1.6 then
		constrib=30*lvl^1.6
	end
    if exploit >30*0.05*lvl^1.6 then
	    exploit=30*0.05*lvl^1.6
	end
...
--返还杀人的等级/战力衰减，victimlvl被杀者等级，killerlvl杀人者的等级，victimbat被杀者战力，killerbat杀人者的战力
--被杀者等级与杀人者等级相差5级以内，随着被杀者等级提高每级增加10%奖励
--被杀者等级大于杀人者5级以上，随着被杀者等级提高每级减少20%奖励
--被杀者等级小于杀人者5级以下，随着被杀者等级降低每级减少15%奖励
--等级衰减最小值为5%
--最后用等级衰减再乘以(被杀者战力/杀人者战力)^n
function treasurefight_getKillRoleReduce(victimlvl, killerlvl, victimbat, killerbat)
	local rate=0
	if (killerlvl-victimlvl)<=5 and (killerlvl-victimlvl)>=-5 then
	    rate=1-(killerlvl-victimlvl)*0.1
	elseif killerlvl-victimlvl>5 then
	    rate=1.25-(killerlvl-victimlvl)*0.15
        elseif killerlvl-victimlvl<-5  then
            rate=2.5+(killerlvl-victimlvl)*0.2
	end
	...
	rate=rate*(victimbat/killerbat)^1.5
